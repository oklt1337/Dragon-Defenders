        ...
        class BeautifyRenderPass : ScriptableRenderPass {
            ...
            static class ShaderParams {
                ...
                static ShaderParams() {
                    ...
                    sfAspectRatio = Shader.PropertyToID("_SunFlaresAspectRatio");
                    ...
                    dofBokehRT = Shader.PropertyToID("_DofBokeh");
                    ...
                    outlineRT = Shader.PropertyToID("_OutlineRT");
                    outlineIntensityMultiplier = Shader.PropertyToID("_OutlineIntensityMultiplier");

                    blurRT = Shader.PropertyToID("_BlurTex");
                    blurMask = Shader.PropertyToID("_BlurMask");
                    blurMaskedRT = Shader.PropertyToID("_BlurMaskedTex");

                    nightVision = Shader.PropertyToID("_NightVision");

                    chromaticAberrationData = Shader.PropertyToID("_ChromaticAberrationData");
                    chromaticTempTex = Shader.PropertyToID("_ChromaticTex");

                    lutPreview = Shader.PropertyToID("_LUTPreview");

                    frameColor = Shader.PropertyToID("_Frame");
                    frameMask = Shader.PropertyToID("_FrameMask");
                }
            }
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData) {
                ...
                if (beautify.directWrite.value) {
                    ...
                    if (beautify.blurIntensity.value > 0) {
                        int blurSource = ApplyFinalBlur(cmd);

                        if (beautify.compareMode.value) {
                            cmd.GetTemporaryRT(ShaderParams.compareTex, sourceDesc, FilterMode.Point);
                            FullScreenBlit(cmd, blurSource, ShaderParams.compareTex, bMat, (int)Pass.Beautify);
                            ...
                            FullScreenBlit(cmd, blurSource, BuiltinRenderTextureType.CameraTarget, bMat, (int)Pass.Beautify);
                        }
                    } else if (beautify.compareMode.value) {
                        cmd.GetTemporaryRT(ShaderParams.compareTex, sourceDesc, FilterMode.Point);
                        RenderTargetIdentifier dest = ShaderParams.compareTex;
                        if (beautify.chromaticAberrationIntensity.value > 0 && beautify.depthOfField.value) {
                            ...
                            dest = ShaderParams.chromaticTempTex;
                        }
                        FullScreenBlit(cmd, source, dest, bMat, (int)Pass.Beautify);
                        if (dest != ShaderParams.compareTex) {
                            // chromatic aberration added as a post-pass due to depth of field
                            FullScreenBlit(cmd, dest, ShaderParams.compareTex, bMat, (int)Pass.ChromaticAberration);
                        ...
                        FullScreenBlit(cmd, source, dest, bMat, (int)Pass.Beautify);
                        if (dest != BuiltinRenderTextureType.CameraTarget) { // add custom chromatic aberration pass
                            FullScreenBlit(cmd, dest, BuiltinRenderTextureType.CameraTarget, bMat, (int)Pass.ChromaticAberration);
                            cmd.ReleaseTemporaryRT(ShaderParams.chromaticTempTex);
                        }
                    }
                    ...
                    if (beautify.debugOutput.value == Beautify.DebugOutput.DepthOfFieldCoC) {
                        ...
                        if (beautify.compareMode.value) {
                            ...
                            FullScreenBlit(cmd, blurSource, source, bMat, (int)Pass.Beautify);
                        }
                    } else if (beautify.compareMode.value) {
                        cmd.GetTemporaryRT(ShaderParams.compareTex, sourceDesc, FilterMode.Point);
                        RenderTargetIdentifier dest = ShaderParams.compareTex;
                        ...
                        FullScreenBlit(cmd, ShaderParams.inputTex, dest, bMat, (int)Pass.Beautify);
                        if (dest != source) { // add custom chromatic aberration pass
                            FullScreenBlit(cmd, dest, source, bMat, (int)Pass.ChromaticAberration);
                            cmd.ReleaseTemporaryRT(ShaderParams.chromaticTempTex);
                        }
                    }
            ...
            int ApplyFinalBlur(CommandBuffer cmd) {
