...
{
    [CreateAssetMenu(menuName = "AbilitySystem/Abilities/DamageAbilities/SingleShotSetRange",
        fileName = "SingleShotSetRange")]
    public class SingleShotSetRangeAbilityObj : SingleShotAbilityObj
    {
        ...
        public void Cast(Transform spawnPoint, Transform target, Caster caster, float abilityDamage,
            float abilityProjectileSpeed, float abilityTravelRange)
        {
            //Spawn projectile
            var projectile = Instantiate(PrefabProjectile, spawnPoint.position, Quaternion.identity)
                .GetComponent<MovingSetRangeProjectile>();
